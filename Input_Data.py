...
def add_noise_classificaiton_model1(X, y, noise_level, C=1.0):
    if noise_level != 0:
        ...
        pos = random.sample(range(length), int(length))
        rank = rank.reshape(rank.size)
        for j in pos:
            y[rank[j]] = -y[rank[j]]
    return X, y
...
def handwritten_digits2(noise_level=0, model=1):
    ...
    :return:
    """
    file = open("/home/xiaohan/baseline/datasets/mfeat-pix.txt")
    Z = file.readlines()
    X = Z[0]
    X = X.strip(' ')
    X = X.split()
...
def handwritten_digits3(noise_level=0, model=1):
    file = open('/home/xiaohan/baseline/datasets/pendigits.txt')
    y = [-1]
    Z = file.readlines()
    X = Z[0]
    X = X.strip()
...
def regression_data5(noise_level=0,model=1):
    file = open('/home/xiaohan/baseline/datasets/regression_1_houses.txt')
    Z = file.readlines()
    y = []
    X = np.ones(8)
    for sample in Z:
...
def regression_data8(noise_level=0,model=1):
    file = open('/home/xiaohan/baseline/datasets/regression_5_liver.txt')
    Z = file.readlines()
    y = []
    X = np.ones(6)
    for sample in Z:
...
def regression_data9(noise_level=0,model=1):
    file = open('/home/xiaohan/baseline/datasets/regression_6_air_craft.txt')
    Z = file.readlines()
    y = []
    X = np.ones(6)
    for sample in Z:
...
def regression_data10(noise_level=0,model=1):
    file = open('/home/xiaohan/baseline/datasets/regression_7_wind.txt')
    Z = file.readlines()
    y = []
    X = np.ones(6)
    for sample in Z:
...
def regression_data12(noise_level=0,model=1):
    file = open('/home/xiaohan/baseline/datasets/house.txt')
    Z = file.readlines()
    y = []
    X = np.ones(6)
    for sample in Z:
...
def regression_data13(noise_level=0,model=1):
    file = open('/home/xiaohan/baseline/datasets/ele.txt')
    Z = file.readlines()
    y = []
    X = np.ones(18)
    for sample in Z:
...
def regression_data17(noise_level=0,model=1):
    data, meta = arff.loadarff('/home/xiaohan/baseline/datasets/ailerons.arff')
    m = np.shape(data)[0]
    data = [list(data[i]) for i in range(0,m)]
    data = np.array(data)
    X = data[:, 1:]
...
def add_noise_classificaiton_model2(X, y, noise_level, C=1.0):
    if noise_level != 0:
        m,n = np.shape(X)
        pos = random.sample(range(m), int(m * noise_level))
        for j in pos:
            y[j] = -y[j]
    return X, y
...
def classification_data5():
    panda = read_arrf('./datasets/fried1.arff')
    X = panda.iloc[:,:-1].values
    y = panda.iloc[:, -1].values
    y[y == 'N'] = -1
    y[y == 'P'] = 1
...
def handwritten_digits7(noise_level=0, model=1,C=1.0):
    file = open('//home/xiaohan/baseline/datasets/hiva.txt')
    Z = file.readlines()
    y = []
    X = np.ones(1617)
    for sample in Z:
